...
#define imgui_xorstr_(str) ::ImGui::jm::xor_string([]() { return str; }, std::integral_constant<std::size_t, sizeof(str) / sizeof(*str)>{}, std::make_index_sequence<::ImGui::jm::detail::_buffer_size<sizeof(str)>()>{})
#define IM_XOR(str) imgui_xorstr_(str).crypt_get()
  ...
  namespace jm {
    ...
    public:
      ...
      XORSTR_FORCEINLINE xor_string(L l, std::integral_constant<std::size_t, Size>, std::index_sequence<Indices...>) noexcept
        : _storage{ ::ImGui::jm::detail::load_from_reg((std::integral_constant<std::uint64_t, detail::load_xored_str8<Size>(Keys, Indices, l())>::value))... }
      {}
      ...
      XORSTR_FORCEINLINE void crypt() noexcept
      {
        ...
        alignas(alignment)
          std::uint64_t arr[]{ ::ImGui::jm::detail::load_from_reg(Keys)... };
        std::uint64_t* keys =
          (std::uint64_t*)::ImGui::jm::detail::load_from_reg((std::uint64_t)arr);
#else
        alignas(alignment) std::uint64_t keys[]{ ::ImGui::jm::detail::load_from_reg(Keys)... };
#endif
